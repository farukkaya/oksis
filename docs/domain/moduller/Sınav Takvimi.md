...
- ~~**Görüş penceresi yarım duruyor.**~~ **Karara bağlandı (2026-09-13):** Faz 2b'de TAM uygulanacak — `SessionReviewComment` varlığı, aç/yorum bırak/çözüldü işaretle komutları, `ExamReviewOpened` bildirimi. Yayın kapısı YUMUŞAK: görüş süresi dolmadan yayın da, açık yorumla yayın da uyarı üretir ve gerekçe ister (`EX-H08` emsali).
- ~~**Kelebekte yoklama.**~~ **Karara bağlandı (2026-09-13):** kelebeğe özel yoklama YOK. Sınav saatinde [[Yoklama Oturumu]] normal akışında kalır, yoklamayı ders öğretmeni alır, gözetmen kâğıt imza listesi tutar; sunucuda tek satır değişmez. Omurga tasarımının "yoklama gözetmenden gelir" kuralı ve `IsInExamSession` denetimi bu kararla düştü.
- ~~**`EX-S04` yalnız oturum kapsamında.**~~ **Kapandı (2026-09-13, Faz 2b):** kural pencere kapsamına alındı ve yayın ön kontrolünde üçüncü yumuşak satır olarak konuşuyor. Gruplama anahtarı fiziksel oda DEĞİL oturumun derslik satırıdır — aynı oda iki oturumda kullanıldığında odaya göre gruplamak iki oturumun şubelerini birleştirir ve uyarı yanlış yerde susardı.
