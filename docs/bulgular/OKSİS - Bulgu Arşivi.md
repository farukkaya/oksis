...
## 35. Merkezî Kurallar Turu — 6 madde (2026-08-31) ✅

Defter sıfırlama turunun Faz 3'ü. Bu bloğun ortak özelliği: **hepsi bir sınıfın
işiydi**, tek ekranın değil ([[yamalama-kabul-degil]]). `K-12` §B'de karara
bağlanmışlardı.
...
✅ **KAPANDI — 2026-08-31** · `oksis-api` @ `61c3d59` · `oksis-ui` @ `aaf0d93` + `d3ea9b7`.

**`X-17` — kural fonksiyona DAL EKLEMEDEN yaşıyor artık.** `MapStatusCode` modül
başına elle yazılan bir zincirdi ve maddenin kendi uyarısı buydu: *"yüzey başına elle
ayırma aynı hatayı bir sonraki modülde tekrar üretir"*. Aile kuralı eklendi —
`<Modül>.…Forbidden` biçimindeki **her** kod 403'tür, kural fonksiyonun başında ve tek
yerde. Duyurular kovasındaki kopya kaldırıldı.
İlk müşteri not modülünün beş yazma handler'ı: `Grades.Forbidden` + Türkçe gerekçe
(okuyabiliyor ama yazamıyor) / `NotFound` (okuyamıyor bile). İstemci tarafında
`DOMAIN_FORBIDDEN_CODE_PREFIXES` üç modülle genişletildi ve **allowlist bilerek
korundu**: sunucunun statü kararı ile istemcinin *cümleyi geçirme* kararı aynı şey
değil — ikincisi mesajın Türkçe olduğunun ölçülmüş olmasını ister (`identity.*` ailesi
mesajında çeviri anahtarı taşır ve geçmemeli; aşırı düzeltme kontrolü testte).

**`TB-78` — sekiz kopya bire indi.** Defter "resolver `CanViewInfo` süzmüyor" diyordu;
ölçüldüğünde sorgunun kendisi **sekiz yerde** kopyalanmıştı (yedi yoklama handler'ı +
`ResolveClassRoomConsumersAsync`). Süzgeci sekiz yere elle eklemek maddenin şikâyetinin
tekrarı olurdu. `ResolveGuardianPersonIdsAsync` eklendi, **varsayılan güvenli tarafta**;
yoklamanın dört handler'ı `includeInfoRestricted: true` geçiyor ve **davranışları
birebir aynı** — değişen tek şey, istisnanın artık bir bayrakla görünür olması. Yeni
modüller (özellikle `GRADE_PUBLISHED`) güvenli varsayılanı miras alıyor.

**`TB-104` — okuma ile yazma aynı kesmeyi konuşuyor.** Her iki okundu-işaretleme
komutu da `WithinActiveSeason` uyguluyor.

**`X-10` — yazılı tercih ölçümle değişti.** `RouteGuard` artık `isLoading` penceresinde
iskelet gösterip bekliyor. İskelet **satır içi stille** yazıldı: `.sk` sınıfı modül
CSS'lerinde tanımlı (`homework.css`, `grade.css`…) ve bu bileşen HER rotayı sarıyor —
sınıfa dayanmak, o stil sayfası yüklenmemiş rotalarda görünmez bir iskelet bırakırdı.

**`X-18` — taşıma sırasında iki tekrar daha bulundu.** Ortak `ChipRow` çıkarıldı ve
kabın davranışı (`flexGrow` · `flexShrink` · `alignItems`) tek yere alındı. Beşinci bir
ölçüm yapıldı: mobilde **beş** yatay şerit var ve **üçünde koruma hiç yoktu**
(`grade-parts`, `grade-entry-screen`, `grade-component-sheet-screen`). Yani dördüncü ve
beşinci tekrar çoktan yazılmıştı; bugün patlamıyorlar çünkü şu an `flex: 1` bir kabın
doğrudan çocuğu değiller. **Çipin görünümü bilerek dışarıda bırakıldı** — üç modülün
çipi farklı görünür (portal rengi, rozet, durum ikonu); ortaklaştırılan şey kap.

### `TB-31` — davranış DEĞİŞMEDİ, karar bir bayat önermeye dayanıyordu

Karar turunda **fail-closed** seçilmişti. Uygulamadan önce ölçüldü ve maddenin
önermesi çürüdü:

- Önbellek bir **hızlandırma katmanıdır**; bastırmanın yetkili kaydı
  `AbsenceSummary.LastThresholdNotifiedAt` + `LastThresholdNotifiedLevel` ikilisidir ve
  **kalıcıdır**.
- Redis tamamen kesintideyken de `ResolveDbFallbackLevel` **aynı 24 saatlik pencereyi**
  ve aynı seviye karşılaştırmasını uygular. Yani *"kesintide veli aynı uyarıyı iki kez
  alır"* senaryosu bugün **gerçekleşmiyor** — mükerrer pencere zaten kapalı.
- Literal fail-closed **daha kötü olurdu**: Redis'in bir saniyelik kesintisinde ilk kez
  eşiğe ulaşan öğrencinin uyarısı da düşerdi. Bastırılan şey mükerrer değil, gerçek bir
  uyarı olurdu.

Maddenin gerçekten açık olan tek ayağı zaten *"kod bugünkü davranışı seçmiş ama
gerekçesi yazılı değil"*dı — gerekçe `AbsenceCalculator`'ın XML doc'una yazıldı.
Geriye kalan tek pencere (kanal gönderimi ile DB izinin commit'i arası) bu maddenin
değil dispatcher'ın bilinen `Debt-N6`'sıdır; çözümü Outbox.

**Kararın uygulanmaması bilinçlidir ve kullanıcıya bildirilmiştir.**

### Yan iş — X-06 deseni bir kez daha

`TB-103`'ün profil sorgusu için **gerçek SQL testi** yazıldı. Sebep: öğrenci yüzünün on
ucu o kapıdan geçiyor ama birim testlerinin hepsi `IClubScope`'u **ikame ediyor** —
üretimdeki TPH `OfType<StudentProfile>()` sorgusu hiçbir testte koşmuyordu.
...
## 48. Sınav Takvimi kapanış turu — 10 madde (2026-09-15) ✅

**Ne oldu:** modülün açık ve karar bekleyen bütün maddeleri tek oturumda derlendi, altı ürün
kararı kullanıcıyla bağlandı ve sıraya konup uygulandı. Tur `oksis-api` `dff0f63b` üzerinde
başladı.

**Bağlanan kararlar (2026-09-15):**

| ID | Karar |
|---|---|
| `TB-154` | Gerekçe isteyen kodlar: `EX-H08` · `EX-S05` · `EX-S07` · `EX-S08` |
| `TB-159` | İkisi de: taşı komutu **+** şube düzenleme yüzeyi |
| `TB-143` | (a) Cümleyi gerçeğe çevir; tür yönetimi `TB-139`'a kalsın |
| `TB-145` | Duyuru okulun tamamına gider |
| `TB-148` | Muhafazakâr paket (istek varsa ret · kelebekte kapalı · bildirim yok) |
| `TB-151`/`TB-144` | Taslakta oturum kurulamaz; ihlal sayacı gösterilmez |

**Turun kendi dersi — iki madde kod yazılmadan kapandı.** `TB-124` ölçüldüğünde `EX-H02`'nin
veritabanı dizininde, `EX-H07`'nin pencere komutunda çoktan uygulandığı görüldü; `TB-129`'un
üçte biri de o gün yazılan başka bir testle kapanmıştı. Plan yazılırken **her madde koda
karşı ölçülmeseydi** üçü için boşuna iş açılacaktı (§46 aynı dersin ilk yarısıdır).

**Ölçülerek bulunan iki gerçek kusur** — ikisi de test yazarken çıktı, ikisi de ürün kodunda:
① `UnplaceExam`'da kapı sırası yanlıştı (bekleyen istekli sınav "hiç yerleşmemiş" görünüyor
ve öğretmen yanlış cümleyi alıyordu); ② test konteynerinde `IExamScheduleReader` kaydı
eksikti ve olay sessizce hiçbir şey üretmiyordu.

**Tarayıcı doğrulaması** (Playwright, gerçek dev verisi, yönetici oturumu): `EX-S09` panoda
12-A'yı adıyla anıyor; oturum taşıma `EX-H01`'e çarpıp dört şubeyi adıyla reddediyor, boş
hücrede geçiyor, gözetmenleri yeni saatten türetiyor, elle yazılanları koruyor ve şube başına
denetim satırı yazıyor. Taşınan oturum sonra bulunduğu hâle geri alındı.
...
✅ **Kapandı** (2026-09-15) — kullanıcı kararı: gerekçe isteyen kodlar `EX-H08`, `EX-S05`,
`EX-S07`, `EX-S08`. `EX-S07` soruda sayılmamıştı; Faz 2b beyin fırtınasının 4 numaralı
kararı ("görüş süresi dolmadan yayın → gerekçe yazarak geçer") gereği listeye alındı.

Tanım tek yerde: `ExamRuleInspector.RequiresPublishReason`. Ekran kendi tablosunu TUTMUYOR —
cevap `ExamViolationDto.RequiresReason` ile telden geliyor. Bu, defterin önerdiği
`packages/core` yükleminden daha dar bir çözüm: core'a yazılsaydı liste yine İKİ yerde
(C# ve TS) durur ve üçüncü kodda yine ayrışırdı.

Yan kazanç: pano KPI'ı ve yumuşak uyarı bloğunun alt satırı da dürüstleşti — "gerekçeyle
geçilir" cümlesi artık yalnız gerçekten gerekçe isteyen bir kod varsa yazılıyor.

`oksis-api` `854b074d` · `oksis-ui` `bfbb482`. Üç birim testi; biri yüklem ile telin aynı
cevabı verdiğini kilitliyor.
...
✅ **Kapandı** (2026-09-15) — kullanıcı kararı ①+② birlikte.

**① `MoveExamSession`** (`PATCH sessions/{id}/slot`): oturum bütün şubeleriyle taşınır,
derslik/gözetmen/sıra yeni saate göre yeniden türer, yayındaki taşıma gerekçe ister ve şube
başına denetim satırı + `ExamMoved` bildirimi düşer. `ExamRevisionKind.SessionMoved = 5`.

**② Şube düzenleme yüzeyi**: `UpdateSessionSections`'ın ilk çağıranı; son şube çıkınca
oturum siliniyor ve ekran listeye dönüyor.

**Sıra kilidi ölçüldü:** taşıma kaydedilmeden bestekâr çağrılırsa gözetmen ESKİ saatten
türüyor. Bilerek bozulup test kırmızı doğrulandı, sonra düzeltildi.

**Tarayıcıda gerçek veriyle doğrulandı:** 8 Ekim'e taşıma `EX-H01`'e çarptı ve dört şubeyi
ADIYLA reddetti; boş hücreye (7 Ekim 4. ders) taşıma geçti; türetilen gözetmenler yeni
saatin öğretmenlerine döndü (Furkan Polat → Deniz Çetin, Levent Koç → Melike Şen), **elle
yazılanlar korundu** (Kısıt 19), dizilim değişmedi (`TB-161` — tuz oturum kimliği), 4 denetim
satırı yazıldı (`kind=5`, sürüm 13→14). Sonra veri bulunduğu hâle geri alındı.

`oksis-api` `1cd24b83` · `oksis-ui` `bfbb482`. 12 entegrasyon testi.
...
✅ **Kapandı** (2026-09-15) — iki yeni `NotificationKind` (42/43), defterin önerdiği adlarla.

`ExamReviewCommentAdded` → okulun sınav yöneticileri (tek kişi değil: `CreatedByPersonId`
oturumun sahibi değil izidir ve o kişi ayrılmış olabilirdi).
`ExamReviewCommentResolved` → **yalnız yorumun sahibi**; çözüm notu gövdeye giriyor.

İkisi de `Info` ve "bekleyen iş" DEĞİL; push haritasına GİRMEDİ — defterin dediği gibi zil
yeter. Katalog satırları + göç yazıldı ve dev veritabanına uygulandı.

**Gövde yorumun METNİNİ taşımıyor:** görüş 1000 karaktere kadar serbest metindir, kırpılmış
bir alıntı yanlış anlaşılır. Haber "bir görüş var, git oku"dur.

`oksis-api` `45fbfcde`. 4 entegrasyon testi.
...
✅ **Kapandı** (2026-09-15) — **muhafazakâr paket** (kullanıcı kararı). Üç açık ayrıntının
cevabı: ① bekleyen istek varsa geri alma REDDEDİLİR ② kelebekte öğretmene açılmaz
③ bildirim yok.

`UnplaceExamCommand` + `POST exams/{id}:unplace` + öğretmen saat seçicisinde
"Saati geri al" düğmesi. Satır SİLİNMİYOR: çift panoda kalıyor ve `EX-S05` onu yeniden
sayıyor.

**Testi yazarken gerçek bir sıra kusuru bulundu:** `PlaceBorrowedHour` `PlacementState`'i
`Unplaced` bırakıyor (K-8), yani istek bekleyen sınav "hiç yerleşmemiş" görünüyordu ve
öğretmen yanlış cümleyi alıyordu. Kapı öne alındı, ölçüt `Date`'e çevrildi.

**Reddetme cümlesi OLMAYAN bir kapıya yollamıyor:** isteği geri çekmenin bir yolu bugün yok
(ölçüldü) — cümle bu yüzden "önce geri çek" demiyor.

⬜ **Tarayıcıda doğrulanamadı:** dev verisindeki üç pencerenin üçü de takvim yayında,
düğmenin görüneceği durum yok; yeni pencere de kurulamıyor (`TB-143`'ün sınırı). Sunucu
tarafı 9 birim testiyle kapalı.

`oksis-api` `5dbbd63f` · `oksis-ui` `32a19e1`.
...
✅ **Kapandı** (2026-09-15) — kullanıcı kararı: **kapsam okul geneline çıktı.**

`ExamNotificationAudience.WholeSchoolAsync`: aktif sezonun bütün şubelerindeki öğrenci ve
veliler + okulun bütün öğretmenleri. Şube kümesi pencerenin SEZONUNDAN geliyor (dönemden
türetmek aynı sezonun ikinci döneminde kümeyi boşaltırdı).

"Beklenen çift yoksa sus" kapısı da kalktı: sınav haftası yerleştirme başlamadan da
duyurulacak bir olgudur — iki adımlı yayının ilk adımının anlamı budur.

Böylece `ExamSchedulePublished`'ın "bundan DARDIR" cümlesi karşılık buldu ve push'un kapsam
dışı bırakılma gerekçesi (okul geneli fan-out) ölçümle de doğru hâle geldi. Enum dokümanına
işlendi.

`oksis-api` `45e466b6`. 2 entegrasyon testi.
...
✅ **Kapandı** (2026-09-15) — kullanıcı kararı **(a)**: cümle gerçeğe çevrildi.

> *"Bu dönemde açılabilecek sınav türü kalmadı — dönemin bütün türleri için pencere zaten
> açılmış."*

Sınav türü yönetimi `TB-139`'un açacağı platform izin modülüne bırakıldı. **Sınırın kendisi
duruyor** ve testi bloke etmeye devam ediyor: bu turda `TB-148`'in öğretmen düğmesi tam bu
yüzden tarayıcıda doğrulanamadı — yeni pencere kurulamadığı için gerekli pencere durumu
üretilemedi.
...
✅ **Kapandı** (2026-09-15) — kullanıcı kararı **(a)**: taslakta oturum kurulamaz.

Düğme taslakta çizilmiyor; boş durum metni artık mod körü değil ("oturumu YÖNETİCİ kurar",
"öğretmenler yerleştirsin" değil). **Kapı sunucuda da kuruldu** — kararın ikinci yarısı buydu
ve olmasaydı düz bir uç çağrısı kuralı yok sayardı (`TB-32`).

Sunucu kapısı tohumların çoğunu ısırdı: **121 test kırmızıya döndü** çünkü hepsi pencereyi
taslakta bırakıyordu. Hepsi ön koşulu sağlayacak biçimde düzeltildi; 365 sınav entegrasyon
testi yeşil.

`oksis-api` `6b9d7fc1` · `oksis-ui` `bfbb482`.
...
✅ **Kapandı** (2026-09-15) — kullanıcı kararı **(a)**: taslakta ihlal sayacı gösterilmiyor.
Taslağın anlamını kart zaten `0/N · %0` ile söylüyor. `oksis-ui` `bfbb482`.
...
✅ **Kapandı** (2026-09-15) — **kod yazılmadı, ÖLÇÜLDÜ.** İki kod da zaten uygulanıyordu,
yalnız bu sınıfın dışında:

- `EX-H07` → `CreateExamWindowCommandHandler:48`, tarih aralığı kesişen ikinci pencereyi
  `Conflict` ile reddediyor.
- `EX-H02` → **yapısal**: `ux_scheduled_exams_window_section_subject` tekil dizini
  `(okul, pencere, şube, ders)` üzerinde, `is_deleted = 0` süzgeçli. Dev veritabanında
  doğrulandı. Faz 1'in tembel satır modelinde satırın KİMLİĞİ zaten bu dörtlü; ikinci satır
  kavramsal olarak da yok. Yazma yollarının ikisi de satırı önce ARIYOR, yani dizine hiç
  çarpılmıyor.

Dizinin gerçekten ısırdığı bağımsız olarak görüldü: `TB-129` testi ikinci bir satır kurmaya
çalışınca SQL reddetti.

Gerekçe `ExamRuleInspector`'ın kod tablosuna yazıldı — "eksik" ile "başka katmanda" ayrımı
yoruma geçmezse madde altı ay sonra yeniden açılır. `oksis-api` `b684b58c`.
...
✅ **Kapandı** (2026-09-15) — üçünün de testi yazıldı, defterin önerdiği desenle.

- `ExamWindowPublished` → `TB-145` turunda iki test
- `ExamHourRequested` → yalnız EV SAHİBİNE gider, şubeye gitmez
- `ExamHourAnswered` → yalnız İSTEYENE gider; kabul/ret/süre dolması ÜÇ AYRI gövde

**Kayıt eksiği bulundu:** test konteynerinde `IExamScheduleReader` yoktu; eksikken MediatR
saat isteği işleyicisini hiç kuramıyor ve olay sessizce hiçbir şey üretmiyordu. Kayıt
prod'daki gibi tamamlandı — yoksa test "bildirim çıkmadı" ile "işleyici bağlanamadı"yı
ayırt edemezdi (Kısıt 2'nin aynı dersi).
...
> ✅ **KAPANDI (2026-09-15).** Elenen şube artık **adıyla görünüyor**: yeni yumuşak
...
>
> ✅ **Sihirbaz ayağı da kapandı** — kullanıcı kararı (2026-09-15): **seçilemez satır değil,
> dipnot.** Defterin önerdiği "satır dursun, seçilemez olsun" biçimi elenen kümeyi TELDE
> şube × ders çifti olarak taşımayı gerektiriyordu; sihirbazın adımı ders eksenli, sunucunun
> olgusu ise şube eksenli. Yeni bir tel alanı açmak yerine adlar panonun ZATEN okuduğu
> `EX-S09` satırından (`sectionName`) geçiriliyor.
>
> Sihirbazın şube adımında: *"1 şube ders programı yayınlanmadığı için listede yok: **12-A**.
> Programı yayınlanana kadar bu şube sınav takvimine hiç girmez."* Tarayıcıda doğrulandı;
> liste 5 şube gösteriyor ve altıncısının nereye gittiği artık soru değil.
>
> Dipnot liste BOŞKEN de çiziliyor — elenen tek şubeyse "bu derste şube kalmadı" cümlesi tek
> başına yanıltıcıdır.
>
> `oksis-api` `b684b58c` · `oksis-ui` `bfbb482`, `aecfd09`. 4 entegrasyon testi.
...
| Oturum sihirbazı, şube adımı | 12-A satırı **hiç yok** | ✅ dipnot: elenen şube adıyla söyleniyor |
...
## 52. `K-28` Kurum Yetkilisi — platform sorar, platform düzenler (2026-09-22) ✅

**Karar:** `K-28 (a)` (2026-09-15) — *kurum yetkilisini platform açılışta sorar, sonradan
platform düzenler, müdür yalnız görür.* Uygulama planı
[[2026-09-16-kurum-yetkilisi-k28]] (Adım 1–8); Adım 1 ve 2 2026-09-16'da, kalanı bu turda.

**Tetikleyen:** kullanıcının okul açma ekranı üzerindeki talebi — *"Kurum Kimliği, İletişim
Bilgileri kartlarındaki bilgileri okul kaydında kullanıcıya soralım… Ek olarak Kurum Yetkilisi
bilgisini de bu forma ekleyelim… ve her okul satırında bir düzenle butonu olsun."*
Talep, `K-28`'in bekleyen üç adımıyla birebir örtüştü.

### Kullanıcı kararları (2026-09-22)

| # | Soru | Karar |
|---|---|---|
| 1 | Hangi künye alanları okul açmayı engellesin | **"DB'de zorunlu alanlar formda da zorunlu olsun"** |
| 2 | Satırdaki "Düzenle" neye dokunsun | **Ad ve iletişim bilgileri** (kod/tür/program kilitli) |
| 3 | Emeklilik ayağı bu turda mı | **Evet** — uç ve izin silinir |

**1. kararın ölçülmüş sonucu:** `oksis_dev` · `INFORMATION_SCHEMA.COLUMNS` taramasında
`official_name`, `meb_code`, `display_name`, `ownership_type`, `founding_year`,
`contact_info_*`, `address_*` ve `authority_*` — **hepsi `IS_NULLABLE = YES`**. Yani kuralın
doğal sonucu: *hiçbir yeni alan zorunlu değil.* Zorunlular `school.schools` tarafındaki
`name`/`code`/`type` ile programda ve müdür alanlarında kaldı. Plan yetkiliyi zorunlu
varsaymıştı; **kullanıcı kuralı planın varsayımını geçersiz kıldı** ve kural uygulandı.

### Yapılanlar

**Ortak künye şekli (yeni).** `SchoolProfileInput` + `SchoolProfileApplier` +
`SchoolProfileInputValidator` (`Modules/Platform/Common/`). Açma ve düzenleme AYNI kayıttan ve
AYNI uygulayıcıdan geçiyor. Gerekçe doğrudan `TB-171`'in kendisi: iki yol ayrı ayrı yazsaydı
biri büyüdüğünde öteki sessizce geride kalır ve "açılışta sorulan ama düzenlenemeyen alan"
sınıfı yeniden doğardı. Doğrulayıcıdaki uzunluklar kolonlardan birebir alındı — kullanıcı 422
görsün diye, "String or binary data would be truncated" 500'ü değil.

**Adım 3 — açılış formu sorar.** `CreateSchoolCommand` + `Profile` (opsiyonel); handler künyeyi
`UpdateBasicInfo`/`UpdateContactInfo`/`UpdateAddress`/`UpdateAuthority` ile yazıyor. Göç
gerekmedi: kolonlar zaten vardı ve nullable.

**Adım 4 — platform düzenleme ucu.** `UpdateSchoolProfileCommand` (`PlatformOnly`) →
`PUT platform/schools/{id}`, ve formu dolduran `GetSchoolProfileQuery` →
`GET platform/schools/{id}`. Handler okul satırını doğrudan okuyor, ayarlara dokunmadan önce
`SetForLoginFlow` ile kurulum bağlamı kuruyor (planın uyardığı tuzak: `IgnoreQueryFilters` ile
güncelleme, yazma denetçisine ve yanlış önbellek önekine yol açardı) ve sonunda
`school-settings` önbelleğini temizliyor.

**Adım 5 — K5 ucu ve izni emekli.** Silindi: `PUT school-settings/authority` aksiyonu,
`Modules/Schools/Commands/UpdateSchoolAuthority/` (komut + doğrulayıcı + handler), birim testi,
`PermissionSeedData` satırı, `MasterSeedIds` GUID'i, `RolePermissionSeedData` eşlemesi. Göç
`20260921220731_20260922_retire_manage_authority_permission` iki satırı düşürüyor
(`role_permissions` `516b702a-…`, `permissions` `a0e409f4-…`); `Down()` ikisini de geri yazıyor.
Dev DB'ye uygulandı. Tombstone (410/403) **yazılmadı** — depo emsali silme yönünde ve
`TB-165` ucun zaten çağrılamadığını ölçmüştü.

**Adım 6 — platform ekranı.** Sayfadaki sabit "Yeni okul" kartı kalktı; sağ üstte **Okul ekle**,
liste tam genişlikte, her satırda **Düzenle**. Yeni `school-wizard.tsx` üç adımlı sihirbazı
açma ve düzenleme kipiyle birlikte taşıyor (kimlik → iletişim & adres → yetkili [+ müdür]).
Düzenleme kipinde kod, tür ve MEB programı kilitli ve **neden kilitli olduğu ipucunda yazıyor**;
boş bırakılmıyor, çünkü boş alan "doldurulabilir" demektir.

**Adım 7 — ölü sözleşme.** Tip sistemi kendiliğinden yakaladı: codegen sonrası
`packages/api` `updateAuthority` uç fonksiyonu derlenmedi. Onunla birlikte `useUpdateAuthority`
kancası ve `packages/core` `UpdateAuthorityInput` tipi silindi (çağıranı yoktu).
Kilit ipucu web'de **"Kurum yetkilisini OKSİS platformu düzenler"** oldu ve rozet **koşulsuz**
gösteriliyor — eskiden `!sup` koşuluna bağlıydı, oysa artık okul içinde kimse düzenleyemiyor.
Mobilde (`school-identity-screen`) aynı cümle.

### Kapanan maddeler

`TB-171` · `TB-165` · `TB-172` — blokları aşağıda.

`TB-165` **ortadan kalktığı için** kapandı, erişim açıldığı için değil: izin de onu isteyen
komut da silindi, yetenek platform yüzeyine taşındı ve orada izin çözümü hiç kullanılmıyor.
`MasterRoleSeedTests`'teki `superAdminOnlyCodes` istisnası da temizlendi — bırakılsaydı bekçi
artık var olmayan bir kuralı savunuyor gibi okunurdu. `SchoolSettingsControllerTests` uç sayımı
**42 → 41**.

⚠️ **Boşluk yok olmadı, ERTELENDİ:** `AccountPermissionResolver.MapProfileToPortal` hâlâ
`Platform` portalını hiçbir profile eşlemiyor. Bu turda DEĞİŞMEDİ ve değişmemeliydi — platform
isteği bu çözücüden geçmiyor. Ama `0019`'un üç platform rolü gerçek izin denetimi isteyince
platform tarafına ayrı bir izin çözücü gerekecek. Bu yüzden `TB-165` kapanırken portal
süzgecinin `Platform` körlüğü defterde **ayrı bir satır** olarak yaşatıldı.
...
## 58. `TB-243` + `TB-242` · Öğretmen kapasite varsayılanları okul ayarında (2026-09-23) ✅

### `TB-243` · Varsayılan haftalık kapasite koda gömülü; okul ayarından, sınıf ve branş öğretmeni için ayrı girilmeli 🟠 *(kapandı — 2026-09-23)*
...
✅ **Karar (2026-09-23, kullanıcı):** (a) MEB *Sınıf Öğretmenliği* **branşı**. Alan yalnız okulda
**ilkokul kademesi açıksa** sorulur; değilse ayarlarda yalnız branş öğretmeni varsayılanı görünür.
Varsayılanlar **okul bazlı, kalıcı** (`SchoolSettings`, sezona bağlı değil). Uygulama yorumu: branş
öğretmeni alanı her okulda vardır (ilkokulun İngilizce/din öğretmeni de onu alır); sınıf öğretmeni
değeri yalnız Sınıf Öğretmenliği branşındaki öğretmene uygulanır.

~~**Karar gerekiyor:**~~ *(yukarıda bağlandı)*
...
### `TB-242` · Kapasite sütunu, yükü olmayan öğretmende kişisel kapasiteyi göstermiyor 🟡 *(kapandı — 2026-09-23)*
...
~~Kapatma yolu:~~ uygulandı — kişisel kapasite, yükten bağımsız olarak öğretmen satırına girsin. İki seçenek:
...
✅ **Kapanış (`oksis-api` `ee2ee4a9` · `oksis-ui` `228db6e`):**
- **Karar uygulandı:** `SchoolSettings.BranchTeacherDefaultWeeklyCapacity` /
  `ClassTeacherDefaultWeeklyCapacity` (okul bazlı, kalıcı; göç `20260923_tb243_teacher_capacity_defaults`,
  mevcut okullar 40/40 ile başlar, bugünkü davranış korunur). Aralık 1–40, sınır dışı değer reddedilir.
- **Tek çözücü:** `TeacherCapacityResolver`. Kural: özel profil değeri ?? (ilkokul kademesi açık **ve**
  branş adı Türkçe katlanarak "Sınıf Öğretmenliği" ise sınıf varsayılanı, değilse branş varsayılanı).
  Üç tüketici (`GetTeacherWorkload`, `PublishReadiness`, `CompetencyAssignmentSource`) buradan okur.
  Branş MEB kodu taşımadığı için tanıma **adla**; dev DB'de ad birebir "Sınıf Öğretmenliği" (ölçüldü).
- **Uç:** `GET/PUT school-settings/teacher-capacity-defaults`; `classTeacherApplies` sunucunun kararı,
  ilkokul kapalıysa PUT sınıf değerini göndermez ve kayıtlı değer korunur. Değişiklik yük önbelleğini
  temizler (`WorkloadCacheInvalidationHandler`).
- **TB-242:** yük özeti `capacities` listesiyle kadronun tamamının kapasitesini, kaynağını (`isCustom`) ve
  öğretmenin kendi varsayılanını taşır. İstemci rozeti sayı karşılaştırarak türetmiyor; sabit yalnız özet
  okunamadığında yer tutucu.
- **Ekran:** *Ayarlar › Akademik Politikalar › Öğretmen Kapasitesi* kartı.
- **Ölçüm (canlı, s1 seed okulu — Altınay'a dokunulmadı):** lise hâlinde kartta yalnız branş alanı;
  30 kaydedildi, DB 30, Öğretmenler listesinin tamamı "30 · Vars.". Ürün uçlarıyla 1. sınıf kademesi açılıp
  bir öğretmen Sınıf Öğretmenliği branşına alındı: sınıf alanı göründü, 24 kaydedildi, o öğretmen
  "24 · Vars." ve çekmecede "Varsayılan: 24 saat", ötekiler 30. Özel 20 girilince 20/Özel (varsayılan 24),
  0 ile sıfırlanınca 24'e döndü. Kurulum geri alındı (kademe, branş, öğretmen branşı, 40/40).
- **Testler:** birim takımları yeşil (Domain 1233 · Application 3174 · Api 463); yeni: domain aralığı,
  çözücü (6), komut/sorgu (3), önbellek süzgeci (3), yük özeti `capacities`, istemci eşlemesi (3).
  Entegrasyon 698/1546 kırmızı: kayıtlı `TB-231` tabanı 688 + ClamAV kapsayıcısı açılmadığı için 10;
...
## 61. Altınay saha testi — Y-04 / Y-05 turunda kapanan 13 madde (2026-09-26) ✅

> Hepsi master'a merge edildi ve blokta ölçüldü. Deftere açık kalanlar: `TB-253` (yalnız ekran ölçümü), `D-31` (kısmi), `B-79` (commit bekliyor), `B-71`, `B-78`.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — `oksis-api` `49a0185e`, `oksis-ui` `7c23891`; merge `d6598101` / `89efad2`. Ölçüm blokta.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — Y-05 olarak uygulandı — `oksis-api` `49a0185e`, `oksis-ui` `7c23891`; merge `d6598101` / `89efad2`; tasarım [[sinif-rehberligi-dersleri-ve-sabit-yerlesim]]. Ölçüm blokta.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — `oksis-api` `fe1a6caa` (Y-04), merge `14b1f130`; `oksis-ui` merge `1f0789a`. Ölçüm blokta.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — `oksis-api` `49a0185e` (`GreedyRepair`, `missing_hours` Cuma), `oksis-ui` `7c23891`; merge `d6598101` / `89efad2`. Ölçüm blokta.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — `oksis-ui` `7c23891`, merge `89efad2`. Ölçüm blokta.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — `oksis-api` `49a0185e`, merge `d6598101`. Ölçüm blokta.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — `oksis-ui` `bd5d632` (master, push edildi). Ölçüm blokta.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — `oksis-api` `2e0c54b8`, `oksis-ui` `b475b16`; merge `14b1f130` / `1f0789a`. Ölçüm blokta.
...
✅ **KAPANDI (2026-09-26 arşiv turu):** master'a merge edildi — `oksis-api` `6647bbd3`, `oksis-ui` `9205250`; merge `14b1f130` / `1f0789a`. Ölçüm blokta.
