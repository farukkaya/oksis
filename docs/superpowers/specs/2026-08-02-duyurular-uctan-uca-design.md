...
| **C5-6** | **Acil kapısının TÜRKÇE MESAJINI hiçbir test korumuyor** | Kapı iki testle çivili ama ikisi de **yalnız kodu** ölçüyor: `ResultExtensionsAnnouncementsTests.Should_Map403_When_UrgentIsNotPermitted` sadece `StatusCode == 403` iddia ediyor, `CreateAnnouncementTests` negatif kolu `.WithMessage("*Announcements.Urgent.Forbidden*")` ile yalnız kod parçasını eşliyor (ikisi de okundu 2026-08-10). *"Acil işareti yalnız okul yönetimi tarafından kullanılabilir."* cümlesini boşaltan bir mutasyon **hiçbir testi düşürmez** — yetki kapısının kullanıcıya dönük tek cümlesi korumasızdır |
