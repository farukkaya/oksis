...
| **C5-5** | ✅ **KAPANDI — C6 (2026-08-10): kapı oluşturma anından YAYIN anına taşındı** | **Eski tespit:** kullanıcı kararı (2026-08-10) `Urgent = true`'yu `announcements.approve` iznine bağlamıştı; kapı yalnız `CreateAnnouncementCommandHandler`'daydı, `ApproveAnnouncementCommandHandler` ve `PublishScheduledAnnouncementsJob` `Urgent`'ı hiç sorgulamıyordu, dolayısıyla kapıdan ÖNCE yazılmış yetkisiz acil kayıt **acil olarak yayına çıkardı**.<br><br>**Şimdi:** karar tek bir saf yükleme çıkarıldı (`AnnouncementUrgentGuard.RevokeIfUnauthorized`) ve **iki yayın yolunda da** çağrılıyor — `PublishScheduledAnnouncementsJob` (zamanlanmış kol) ve `ApproveAnnouncementCommandHandler` (onay kolu). Yetkisiz yayınlayanın bayrağı düşürülür, denetim izine warning tonlu bir kayıt düşer (biçimi kola göre değişir — aşağıya bakınız), duyuru yine yayınlanır (fail-closed ama akış kesilmez). Kapı **`Publish()` ÖNCESİNE** konmak zorundaydı ve sıra her iki kolda da ölçülerek doğrulandı: `Announcement`'ın yayın olayı `Urgent`'ı taşır ve `Publish()` içinde fırlatılır — sonrasında düşürülen bayrak bildirim zincirine yetişemezdi. Kapıdan önce yazılmış bekleyen kayıtlar için günlük `SweepUnauthorizedUrgentAnnouncementsJob` (`"0 5 * * *"`, İstanbul TZ — backfill'in idempotent hâli).<br><br>**Denetim izi ASİMETRİKTİR ve bu bilinçlidir.** Onay kolunda gerçek bir aktör vardır (onaylayan), bu yüzden mevcut *"duyuruyu onayladı"* iz satırına yalnız bir `tag` damgası basılır (*"Acil işareti yetkisiz olduğu için kaldırıldı"*) — ikinci satır açılmaz. Zamanlanmış yayın kolunda aktör **yoktur** (job'ın oturumu yok) ve domain `Guid.Empty` aktörünü yasaklar; bu yüzden orada AYRI bir iz satırı yazılır: `action: "acil işareti kaldırıldı"`, aktör yayınlayandır. Asimetrinin gerekçesi budur, üslup tercihi değil. Bkz. **C6-3** — bu değer modülün ilk **edilgen** `action`'ıdır (kardeş yedisi etken: "duyuruyu yayınladı", "duyuruyu onayladı", "duyuruyu reddetti", "duyuruyu düzeltti", "duyuruyu geri çekti", "geri çekmeyi geri aldı", "duyuruyu onaya gönderdi" — ölçüldü 2026-08-11) |
| **C5-6** | **Acil kapısının TÜRKÇE MESAJI korumasız DEĞİL, ERİŞİLEMEZDİ — taşıma yolu açıldı, backend koruması hâlâ açık** | **Düzeltme (2026-08-10).** Bu satırın ilk hâli sorunu eksik teşhis ediyordu: cümle yalnız testsiz değildi, **hiçbir ekrana ulaşmıyordu**. Ölçüm: `oksis-ui` `packages/api` `client/mutation-error.ts` `case 403` **koşulsuz** *"Bu işlem için yetkiniz yok."* döndürüyor, sunucunun `errors[0].message`'ını atıyordu; duyuru yüzeylerinin hiçbiri `err.message`'ı doğrudan taşımıyor. Yani *"Acil işareti yalnız okul yönetimi tarafından kullanılabilir."* **üründe hiçbir yerde görünmüyordu** — üstelik `ResultExtensions.MapStatusCode` docblock'u tam tersini (istemcinin çıplak 403 görmesini ENGELLEMEK için `Result<T>.Forbidden()` kullanılmadığını) iddia ediyordu. **Şimdi:** 403 kolu ikiye bölündü ve bölmeyi **hata kodu** yapıyor — kod `DOMAIN_FORBIDDEN_CODE_PREFIXES` (bugün yalnız `Announcements.`) ile başlıyorsa sunucunun Türkçe cümlesi geçer, jenerik kodlarda sabit cümle konur. Ayrımın **kodla** yapılması zorunluydu, statüyle değil: jenerik yol `AuthorizationBehavior`/`TenantContextBehavior` → `ForbiddenException` (kod `Forbidden`, mesaj *"Access denied."* / *"This operation requires SuperAdmin privileges."*), `Result.Forbidden()` → `Error.Forbidden` + *"Access denied."*, `TenantRequired` → *"This operation requires a tenant context."* — koşulsuz geçirmek bu **İngilizce** metinleri ekrana basardı. Sezgisel bir *"kodda nokta varsa modül kodudur"* kuralı da yanlış olurdu: `identity.account.suspended` noktalıdır ama mesaj alanında bir **çeviri anahtarı** taşır (`identity.errors.account-suspended`, `AccountErrors.cs`) ve ekrana ham anahtar sızardı — bu yüzden kural açık bir önek **listesidir**. **Dört** yeni testle çivili (`mutation-error.test.ts`, dosya **12 → 16** `it` bloğu — ölçüldü 2026-08-10, commit `f012999`; api 135 → 139 ile tutarlı); üç mutasyon ölçüldü: geçirme kolu kaldırılınca 1 test, jenerik kol geçirir yapılınca 3 test (biri C5 öncesinden), boş-mesaj koruması kaldırılınca 1 test düşüyor. ✅ **AÇIK KALAN BACKEND KOLU DA KAPANDI — C6 (2026-08-10), ama planın ÖNCÜLÜ DÜZELTİLEREK.** Bu satırın ilk hâli borcu **iki testte** görüyordu; ikinci ayak yanlıştı. Ölçüm: `ResultExtensionsAnnouncementsTests` bir **mesaj borcu DEĞİLDİR** — o birim testi `Error`'ı kendi literaliyle kurar, üretim sabitini **hiç tüketmez**; ölçtüğü şey cümlenin doğruluğu değil, `Announcements.` önekinin 403'e eşlenmesidir. Duyuru kolunun mesaj borcu **tek yerdeydi** (`CreateAnnouncementTests` negatif kolu) ve orada kapatıldı: iddia artık kodu değil, `AnnouncementPublisherAuthority.UrgentForbiddenMessage`'in ürettiği Türkçe cümleyi eşliyor. **Mutasyonla ölçüldü** (2026-08-10): sabit boşaltılınca **3** test düşüyor (bu görevinki + Görev 7'nin iki şablon testi) ve hata metni *"Announcements.Urgent.Forbidden — "* oluyor, yani **eski iddia aynı mutasyonda geçerdi** — borcun gerçekliği böyle kanıtlandı. Mutasyon geri alındı; commit yalnız test dosyasını taşıdı. **Kalan minör kırılganlık:** eşleme joker (`*cümle*`) olduğu için cümlenin SONUNA metin yapıştıran bir mutasyonu yakalamaz; ham `Result` kardeşi mevcut olduğu için tam eşitlik mümkündür |
...
| **C5-8** | ✅ **KAPANDI — C6 (2026-08-10). ÜRÜN KARARI: TASLAK KİŞİSELDİR** | **Eski tespit:** `GetAnnouncementsQueryHandler`'da varsayılan bir statü kapısı yoktu (temel sorgu yalnız `a.SchoolId == schoolId`; `scope`/`statuses` süzgeçleri isteğe bağlı), yani öğretmenin taslağı hiçbir filtre verilmediğinde yönetimin envanter listesine düşüyordu — ekran ise *"listede yalnız siz görürsünüz"* diyordu.<br><br>**Karar:** taslak kişiseldir; yönetim yetkisi taslağa erişim vermez, taslağı yalnız yazarı görür. **Kapatma İKİ YÜZEYDEDİR** ve yüklem ikisinde de karakter karakter aynıdır — envanter listesi (`GetAnnouncementsQueryHandler`) ve özet sayaçları (`GetAnnouncementSummaryQueryHandler`): `Status != Draft || PublisherId == caller`. İkinci yüzey ilk turda yoktu ve **ölçüm üzerine eklendi**: özet ucu yönetici için hiç daraltma uygulamıyordu (`scope: "school"` kolunda `PublisherId` süzgeci yok), yani liste daralırken `Draft` sayacı okul genelindeki TÜM taslakları saymaya devam ederdi. Yönetici *"Taslak: 3"* kartını görüp tıkladığında 1 satır bulurdu — **bu değişikliğin ÜRETTİĞİ yeni bir kusur** olurdu. Kartlar filtre kısayoludur ve listeyle aynı kümeyi vaat etmek zorundadır; sayacın eklenmesi kararın kapsamının genişletilmesi değil, tutarlı tamamlanmasıdır. **Diğer sayaçlar yapısal olarak etkilenmiyor** (yayınlanmış kayıt asla `Draft`'a dönmez → `UrgentThisMonth` güvende); onay kuyruğu ayrı statü kullandığı için kör kalmıyor. **Detay ucu bilinçli olarak kapsam dışıdır — bkz. C6-7.** Kararın adlandırılmamış ikinci sahnesi için bkz. **C6-6** |
...
### C6'da (2026-08-10) ölçülerek eklenen backlog

**C6 ne yaptı.** Acil (`Urgent`) bayrağının yetki kapısı **oluşturma anından yayın anına
taşındı** (C5-5) ve etrafındaki dört boşluk kapatıldı: zamanlama × moderasyon çakışması
(K2'nin kapanmamış kolu), şablon uçlarındaki acil kapısı, taslak mahremiyeti (C5-8) ve
C5-6'nın backend test borcu. Dilim **şema değiştirmedi** — `has-pending-model-changes`
kapanışta *"No changes"* verdi (ölçüldü 2026-08-11). On görev, her biri ayrı incelemeden
geçti; dal `feature/announcements-c6`.

**Kapanış doğrulaması (ölçüldü 2026-08-11).** `dotnet build` → **0 uyarı / 0 hata**.
`dotnet test` → beş projenin beşi de yeşil, **0 düşen**: `Oksis.Api.UnitTests` 248,
`Oksis.Domain.UnitTests` 688, `Oksis.Application.UnitTests` 1545, `Oksis.Tests` 40,
`Oksis.Infrastructure.IntegrationTests` 881 — **toplam 3402**.

**Atıf kuralı C4/C5'ten devam eder:** satır numarası değil sembol/dosya adı; sayı zorunluysa
yanında ölçüm tarihi. Aşağıdaki maddeler C6 kapsamı dışında **bilerek** bırakıldı;
kapanan maddeler bu tabloda YOKTUR (bayat backlog üretmemek için ayıklandı).

| # | Madde | Ölçüm ve etki |
|---|---|---|
| **C6-1** | **D fazı ön koşulu — acil duyurunun kanal karşılığı henüz TANIMSIZ** | `NotificationEventTypeSeedData` `ANNOUNCEMENT_URGENT`'a `DefaultEmailEnabled: true` verir (normal `ANNOUNCEMENT` `false`), `NotificationConfig.QuietHours*` kalıcı ve düzenlenebilirdir ama **hiçbir teslim yolu okumaz**. `AnnouncementPublished` bildirimini iki olay türünden hangisine eşleyeceğini söyleyen **hiç kod yok**: `ANNOUNCEMENT_URGENT`'ın seed ve seed-id dışında tüketicisi yoktur (ölçüldü 2026-08-10, yeniden ölçüldü 2026-08-11). K-2 kanalları bağlandığı gün "acil" = kanal tırmanması olur; eşleme yazılırken **C6'nın kapıları oraya da uygulanmalıdır**, yoksa yetki kapısı in-app'te tutar ama e-posta/push kolunda sızar |
| **C6-2** | **Onay kuyruğu acil rozetini göstermiyor** — `oksis-ui` maddesi | `apps/web/features/announcements/approval-queue-tab.tsx` içinde `urgent` kelimesi **0** kez geçiyor (ölçüldü 2026-08-10, yeniden ölçüldü 2026-08-11). Onaylayan, acil bir duyuruyu onayladığını ekranda göremez. C6 bunu **yetkisiz** durumda zararsız kıldı (bayrak zaten düşürülüyor), ama **yetkili** acil duyuruda rozet hâlâ eksiktir — yönetici acil olduğunu bilmeden onaylar |
| **C6-3** | **Denetim izinde modülün ilk EDİLGEN fiili** — `oksis-ui` maddesi | `"acil işareti kaldırıldı"` (`PublishScheduledAnnouncementsJob`) bu modülün ilk edilgen `action` değeridir; kardeş yedisi etkendir ("duyuruyu yayınladı", "duyuruyu onayladı", "duyuruyu reddetti", "duyuruyu düzeltti", "duyuruyu geri çekti", "geri çekmeyi geri aldı", "duyuruyu onaya gönderdi" — ölçüldü 2026-08-11). Aktör **yayınlayandır**, çünkü domain `Guid.Empty` aktörünü yasaklar ve job'ın oturumu yoktur. İstemcinin denetim izi çekmecesi cümleyi `"{ActorName} {Action}"` diye kuruyorsa okunuşu bozulur (*"Ayşe Öğretmen acil işareti kaldırıldı"*) — **`oksis-ui`'da gözle doğrulanmalı** |
| **C6-4** | **`SweepUnauthorizedUrgentAnnouncementsJob` GERİ DÖNDÜRMEZ** | Süpürge yalnız `AnnouncementUrgentGuard.RevokeIfUnauthorized` çağırır; bayrağı geri veren hiçbir kol yoktur (dosya okundu 2026-08-11). Yönetici acil bir taslak yazıp geçici olarak iznini kaybederse süpürge bayrağı siler ve izin geri geldiğinde **kendiliğinden geri gelmez**. Yayın anı kapısının kararıyla tutarlıdır (o gün de düşerdi) ama **kullanıcıya hiçbir yerde söylenmiyor** |
| **C6-5** | **Zamanlama × moderasyon kapandı — K2'nin bilinçli DARALMASI** | Eşikli moderasyon modunda öğretmen artık velilere duyuru **zamanlayamaz**: `CreateAnnouncementCommandHandler` `Announcements.Schedule.RequiresApproval` (400) döner, önce onaya göndermek zorundadır. Kapı `MarkScheduled`'ın **tek çağıranındadır**, yani `Scheduled`'a giden tek yol kapalıdır — kaçak kol ölçülerek arandı, yok. Bu bir hata değil, K2'nin kapanmamış kolunun kapatılmasıdır; ama ürün açısından **yeni bir kısıttır** ve akış/ekran tasarımında bilinmelidir |
| **C6-6** | **REDDEDİLEN duyuru yöneticinin BÜTÜN yüzeylerinden kayboluyor** | `Announcement.Reject()` kaydı `PendingApproval`'dan `Draft`'a döndürür ve kaydın sahibi öğretmendir. C5-8 kararından sonra o kayıt yöneticinin **ne envanterinde, ne `Draft` sayacında, ne onay kuyruğunda** görünür. Ürün kararının **lafzıyla tutarlıdır** (taslak kişiseldir, reddedilen kayıt taslaktır) ama kimse bu sahneyi adlandırmadı. *"Reddettiğim duyuru nereye gitti?"* sorusu geldiğinde kararın **bilinçli** olduğu buradan görünsün. Yöneticiye reddettiği kayıtların izini bırakmak istenirse çözüm C5-8'i gevşetmek değil, ayrı bir "reddettiklerim" yüzeyi ya da denetim izi üzerinden okuma olmalıdır |
| **C6-7** | **Detay ucu (`GET /announcements/{id}`) taslak mahremiyeti kapsamına ALINMADI** | C5-8 kapatılırken bilinçli olarak dışarıda bırakıldı. Gerekçe: detay ucu bir liste/sayaç yüzeyi değil, **derin bağlantıyla ulaşılan tekil kayıttır**; görünürlük kararı orada `AnnouncementLifecycleGuard.CanActOn` ekseninde verilir (bugün "kendi kaydı VEYA yönetim") ve o yüklemi değiştirmek denetim izi + gönderim raporu uçlarını da etkiler. Yani ayrı bir iştir ve kendi kararını ister. **Bugünkü sonuç:** yönetici taslağı listede göremez ama kimliğini bilirse detayını çekebilir |
| **C6-8** | 🔶 **Toplu çağıran TEKİL yetki API'sini kullanıyor — PARK EDİLDİ (kullanıcı kararı 2026-08-10)** | `SweepUnauthorizedUrgentAnnouncementsJob` `AnnouncementPublisherAuthority.IsManagerAsync` çağırıyor; o metodun **kendi docblock'u** toplu kullanımda `IsManagerForAccountAsync` önerir ve kardeş job (`PublishScheduledAnnouncementsJob`) o kalıba uyar (ikisi de okundu 2026-08-11). **Planın kendisi çelişiyordu:** bir görevi docblock'u yazdırdı, diğeri tersini. **Kullanıcı hükmü (2026-08-10):** davranış doğrudur ve fail-closed korunur; asıl N+1 (duyuru başına sorgu) yetki önbelleğiyle zaten kapalıdır; kalan artık **günlük** bir job'da farklı (yayınlayan, sezon) başına tek satırlık bir `Persons` sorgusudur — maliyet ihmal edilebilir. Kod değişmedi. **Kalan:** kardeş job'la kalıp farkı bir bakım borcu olarak duruyor |
| **C6-9** | **`announcements.approve` slug'ı modülde hâlâ 5 yerde LİTERAL** | `AnnouncementPublisherAuthority.ManagementPermission` sabiti C6'da açıldı ama modülün **tek kaynağı olmadı**. Literal kalan yerler (migration/seed hariç, ölçüldü 2026-08-11): `AnnouncementCallerResolver`, `AnnouncementSubmittedForApprovalNotificationHandler` ve üç `[RequirePermission]` (`GetAnnouncementApprovalsQuery`, `ApproveAnnouncementCommand`, `RejectAnnouncementCommand`). Attribute'lar sabit ifade istediği için `const` bağlanabilir; asıl engel yok, iş yapılmadı. Slug değişirse beş yerin beşi de elle bulunmalıdır |
| **C6-10** | **DEPO GENELİ — Mapster `TypeAdapterConfig.GlobalSettings` süreç-genelinde tek MUTABLE statik** (C6 ürünü DEĞİL) | `Application/DependencyInjection.cs` yapılandırmayı `TypeAdapterConfig.GlobalSettings` üzerinde kurar (ölçüldü 2026-08-11). Süreç-genelinde tek ve değiştirilebilir olduğu için paralel yük altında `GetDynamicMapFunction` yarışı **yapısal olarak mümkündür**; C6 sırasında bir Schools/Mapster testi tam bu profille flaky ölçüldü (implementer stash'li/stash'siz izole koşularla C6 ile ilgisiz olduğunu ayrıca ölçtü; **kapanış koşusunda tekrarlamadı, 2026-08-11**). Duyuru modülüyle ilgisi yoktur, **ayrı bir iş kalemidir**; burada yalnız kaydı düşürülmüştür ki bir sonraki flaky koşuda sıfırdan teşhis edilmesin |
