...
Bu dosya `oksis` deposunda çalışan Claude Code oturumlarına yol gösterir. `AGENTS.md` aynı içeriği Codex için taşır — biri değişirse öteki de değişir.
...
- Kod repolarının kendi `CLAUDE.md`'si katmana özgü komut ve yasakları taşır; bir katmanda çalışırken önce onu oku.
...
| `domain/` | Modül, kavram ve karar notları — **domain bilgisinin tek kaynağı** | Koddan `domain-map` skill'iyle yazılır (`oksis-api/.claude/skills/domain-map/`); `<!-- generated -->` blok kuralı geçerli |
