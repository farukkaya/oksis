...
- **Faz 3 Dilim-2 Çok-Sınıf Otomatik Üretim (BE+FE) (2026-06-17, K-D2-1…6):** Tasarım:
  ...
  - **Job/kapsam (K-D2):** `GenerationScope` enum (Single/GradeLevel/All) + `int? GradeLevel`; `BranchId` nullable;
    ...
    schoolId, ct)` kapsamı→branchId'lere çözüp **tek joint solve**; dış doluluk = yalnız kapsam-dışı canlı program.
    Migration `20260617_schedule_gen_job_scope` (additive; mevcut satırlar → Single).
