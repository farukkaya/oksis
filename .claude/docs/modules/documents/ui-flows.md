# Documents (Dosya Yönetimi) — UI Flows

> Bu modülün frontend ekranları, kullanıcı akışları, state management.
> **{{TBD}} — Faz 5'te dolacak.** Faz 0-1 backend-only (domain + persistence); web `shared/files` katmanı ve ilk ekran (SchoolLogo swap) spec § 5 ve Faz 5'te gelecek. Mobile B4 kararıyla ertelendi (spec § 6).
