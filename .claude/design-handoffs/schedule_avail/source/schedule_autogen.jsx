...
function AutoGenFlow({ classes, defaultClass, onClose, onUseDraft }) {
  ...
  const [w, setW] = useStateAg({ sabah: 'orta', pencere: 'yuksek', tercih: 'orta', denge: 'orta', blok: true });
            ...
            <React.Fragment>
              ...
              <div className="ag-sec">
                ...
                <div className="ag-weights">
                  ...
                  <div className="ag-weight">
                    <span className="wi"><Icon name="heart" size={17} /></span>
                    <div className="wt"><div className="t">Öğretmen tercihlerine uy</div><div className="s">Öğretmenin tercih etmediği saatlerden kaçınır</div></div>
                    <div className="ag-wseg">{['düşük', 'orta', 'yüksek'].map((v) => <button key={v} className={w.tercih === v ? 'on' : ''} onClick={() => setWeight('tercih', v)}>{v}</button>)}</div>
                  </div>
                  <div className="ag-weight">
