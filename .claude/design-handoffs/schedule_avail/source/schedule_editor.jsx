...
/* ── Öğretmen müsaitliği (schedule_avail'dan gelir) — öğretmen adına göre ──
   'unavail' = Müsait Değil (kesin engel, AŞILABİLİR → onay diyaloğu)
   'prefer'  = Tercih Etmez (yumuşak uyarı, engel değil) */
const SED_TEACHER_AVAIL = {
  'Ahmet Yılmaz': { '4-1': 'unavail', '4-2': 'unavail', '4-3': 'unavail' },  /* Cuma sabah müsait değil */
  'Selin Aydın':  { '3-2': 'prefer', '3-4': 'prefer', '0-7': 'prefer' },     /* Perşembe öğleden sonrayı tercih etmez */
  'Hasan Kılıç':  { '4-6': 'unavail', '4-7': 'unavail' },
  'Leyla Brown':  { '2-8': 'prefer' },
};
/* yerleşmiş hücrenin öğretmen müsaitlik durumu */
function cellAvail(ky, cell) {
  if (!cell) return null;
  const teacher = cell.teacher || SED_SUBJECTS[cell.sub].teacher;
  const m = SED_TEACHER_AVAIL[teacher];
  return m ? (m[ky] || null) : null;
}
/* sürüklenen dersin öğretmeninin hedef hücredeki durumu */
function subAvail(sub, ky, teacherOverride) {
  const teacher = teacherOverride || SED_SUBJECTS[sub].teacher;
  const m = SED_TEACHER_AVAIL[teacher];
  return m ? (m[ky] || null) : null;
}
...
/* ════════════════ Grid hücresi ════════════════ */
function GridCell({ ky, cell, locked, optional, missing, drag, view, tv, availFlag, onDragOver, onDragLeave, onDrop, onOpenMenu, menuOpen, onMenuAction, onMenuClose, flash }) {
  const cls = ['sed-cell'];
  let body = null;
  ...
  if (cell) {
    ...
    if (availFlag === 'unavail') cls.push('avail-override');
    if (availFlag === 'prefer') cls.push('avail-prefer');
    ...
    body = (
      <React.Fragment>
        ...
        {availFlag === 'unavail' && !cell.conflict && <span className="av-badge" title="Müsaitlik aşılarak yerleştirildi"><Icon name="alert-triangle" size={10} strokeWidth={2.8} /></span>}
        <div className="cc-name">{s.ad}</div>
    ...
    return (
      <div className={cls.join(' ') + (drag ? (drag.tone === 'ok' ? ' drop-ok' : drag.tone === 'warn' ? ' drop-warn' : ' drop-bad') : '')} style={{ '--c': s.c }} data-key={ky}
        title={availFlag === 'prefer' ? 'Öğretmen bu saati tercih etmiyor' : availFlag === 'unavail' ? 'Müsaitlik aşılarak yerleştirildi' : undefined}
  ...
  if (drag) cls.push(drag.tone === 'ok' ? 'drop-ok' : drag.tone === 'warn' ? 'drop-warn' : 'drop-bad');
  ...
  return (
    <div className={cls.join(' ')} data-key={ky}
      ...
      {drag && drag.reason && <div className={'drop-tip' + (drag.tone === 'ok' ? ' ok' : drag.tone === 'warn' ? ' warn' : '')}>{drag.reason}</div>}
...
/* ════════════════ "Yine de yerleştir" onay diyaloğu (müsaitlik aşımı) ════════════════ */
function AvailOverrideModal({ cellKey, sub, onClose, onConfirm }) {
  const SModal = window.Modal;
  const [d, p] = cellKey.split('-').map(Number);
  const teacher = SED_SUBJECTS[sub].teacher;
  return (
    <SModal icon="alert-triangle" iconTone="danger" title="Öğretmen bu saatte müsait değil"
      sub={SED_DAYS[d][1] + ' · ' + p + '. ders'} onClose={onClose}
      footer={<React.Fragment>
        <button className="btn btn-ghost" onClick={onClose}>Vazgeç</button>
        <div className="spacer"></div>
        <button className="btn btn-danger" onClick={onConfirm}><Icon name="alert-triangle" size={16} /> Yine de yerleştir</button>
      </React.Fragment>}>
      <div className="aca-note" style={{ marginTop: 0, marginBottom: 0, background: 'var(--danger-bg)', borderColor: 'color-mix(in srgb, var(--danger) 22%, transparent)', color: '#7A1515' }}>
        <Icon name="ban" size={15} style={{ color: 'var(--danger)' }} />
        <span><b>{teacher}</b>, {SED_DAYS[d][1]} {p}. ders saatinde <b>müsait değil</b> olarak işaretli. <b>{SED_SUBJECTS[sub].ad}</b> dersini yine de bu saate yerleştirmek istiyor musunuz? Hücre "müsaitlik aşıldı" rozeti alır.</span>
      </div>
    </SModal>
  );
}
...
function ScheduleEditorScreen({ role, t, onNavigate }) {
  ...
  /* öğretmen müsaitlik ihlalleri — unavail (override, kırmızı) + prefer (tercih, sarı) */
  const availViol = useMemoEd(() => {
    const out = [];
    Object.entries(place).forEach(([k, c]) => { const a = cellAvail(k, c); if (a) out.push({ k, c, kind: a }); });
    return out;
  }, [place]);
  const pubIssues = [
  ...
  function cellDragOver(ky, e) {
    ...
    e.preventDefault();
    const sub = drag.sub;
    let ok = true, tone = 'ok', reason = 'Buraya bırak';
    if (SED_LOCKS[ky]) { ok = false; tone = 'bad'; reason = 'Slot kilitli — bırakılamaz'; }
    else if (SED_BUSY[sub] && SED_BUSY[sub][ky]) { ok = false; tone = 'bad'; reason = SED_SUBJECTS[sub].teacher + ' · ' + SED_BUSY[sub][ky] + "'de meşgul"; }
    else {
      const av = subAvail(sub, ky);
      if (av === 'unavail') { tone = 'bad'; reason = SED_SUBJECTS[sub].teacher + ' · bu saatte müsait değil'; }
      else if (av === 'prefer') { tone = 'warn'; reason = SED_SUBJECTS[sub].teacher + ' · bu saati tercih etmiyor'; }
    }
    if (!over || over.key !== ky || over.tone !== tone) setOver({ key: ky, ok, tone, reason });
  }
  ...
  function cellDrop(ky, e) {
    ...
    const av = subAvail(sub, ky);
    if (av === 'unavail') {
      setModal({ type: 'override', ky, sub });
      setDrag(null); setOver(null); return;
    }
    setPlace((pl) => ({ ...pl, [ky]: { sub } }));
    setDirty(true); setDrag(null); setOver(null);
    if (av === 'prefer') fire(<span><b>{SED_SUBJECTS[sub].ad}</b> yerleştirildi · öğretmen bu saati tercih etmiyor</span>);
    else fire(<span><b>{SED_SUBJECTS[sub].ad}</b> yerleştirildi</span>, 'ok');
  }
          ...
          {SED_DAYS.map((d, di) => {
            ...
            return (
              <GridCell key={view + '-' + ky} ky={ky} cell={cell} locked={view === 'sinif' ? locked : null}
                ...
                availFlag={view === 'sinif' ? cellAvail(ky, cell) : null}
  ...
  return (
    <div className="sched-ed" data-screen-label={'Program Editörü · ' + baseMeta.ad}>
      ...
      <div className="sed-valbar">
        {verifyOpen && !loading && (
          <div className="sed-issues">
            <div className="sed-issues-h">Doğrulama · {conflicts.length + missingCells.length + availViol.length} sorun</div>
            {clean && availViol.length === 0 && <div className="sed-issue-ok"><Icon name="check-circle" size={17} /> Çakışma yok, tüm saatler yerleşti — yayına hazır.</div>}
            {conflicts.map(({ k, c }) => {
            ...
            {availViol.map(({ k, c, kind }) => {
              const [d, p] = k.split('-').map(Number);
              const teacher = c.teacher || SED_SUBJECTS[c.sub].teacher;
              const hard = kind === 'unavail';
              return (
                <button key={'av-' + k} className={'sed-issue ' + (hard ? 'viol' : 'pref')} onClick={() => flashTo(k)}>
                  <span className="ii"><Icon name={hard ? 'ban' : 'minus'} size={15} strokeWidth={hard ? 2 : 3} /></span>
                  <span className="it"><span className="t">Müsaitlik ihlali · {SED_DAYS[d][1]} · {p}. ders — {SED_SUBJECTS[c.sub].ad}</span><span className="s">{teacher} · {hard ? 'müsait değil (aşılarak yerleştirildi)' : 'bu saati tercih etmiyor (yumuşak)'}</span></span>
                  <span className="go">Hücreye git <Icon name="arrow-right" size={13} /></span>
                </button>
              );
            })}
        ...
        <div className="sed-valbar-main">
          <div className="sed-status">
            {loading ? <span className="sch-upd">Doğrulama bekleniyor…</span>
              : <React.Fragment>
                  ...
                  {availViol.length > 0 && <span className="pill viol"><Icon name="ban" size={15} /> {availViol.length} müsaitlik ihlali</span>}
                </React.Fragment>}
          </div>
          ...
          <div className="sed-legend">
            ...
            <span className="sed-leg pref"><span className="sw"></span> Tercih edilmez</span>
      ...
      {modal && modal.type === 'override' && <AvailOverrideModal cellKey={modal.ky} sub={modal.sub} onClose={() => setModal(null)} onConfirm={() => { setPlace((pl) => ({ ...pl, [modal.ky]: { sub: modal.sub, override: true } })); setDirty(true); setModal(null); fire(<span><b>{SED_SUBJECTS[modal.sub].ad}</b> · müsaitlik aşılarak yerleştirildi</span>, 'bad'); }} />}
